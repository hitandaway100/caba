...
class Handler(ScoutHandler):
    ...
    def process(self):
        args = self.getUrlArgs('/sceventlovetime/')
        try:
        ...
        # イベント毎の設定.
        if eventKind in (Defines.ScoutEventType.LEVELUP, Defines.ScoutEventType.COMPLETE, Defines.ScoutEventType.HAPPENING):
            # 結果表示へ.
        ...
        if playdata.result.get('lovetime_start'):
            ...
            self.appRedirectToEffect('scoutevent/lovetime2/effect.html', self.__swf_params)
        else:
            # 演出不要.
