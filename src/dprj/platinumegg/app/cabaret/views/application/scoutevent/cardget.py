...
class Handler(ScoutHandler):
    ...
    def process(self):
        ...
        if itemid:
            if not itemid in Defines.ItemEffect.SCOUT_CARD_ITEMS:
                ...
                if playdata:
                    url = UrlMaker.scouteventresult(stageid, playdata.alreadykey)
        ...
        url = UrlMaker.scouteventcardgetresult(stageid)
        self.appRedirect(self.makeAppLinkUrlRedirect(url))
    ...
    def tr_write(mid, uid, stageid, itemmaster, usenum, autosell_rarity):
        ...
        BackendApi.tr_determine_scoutevent_scoutcard(model_mgr, mid, uid, stageid, itemmaster, usenum, autosell_rarity=autosell_rarity)
        model_mgr.write_all()
        return model_mgr
