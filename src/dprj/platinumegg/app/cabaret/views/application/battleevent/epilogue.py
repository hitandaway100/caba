...
class Handler(BattleEventBaseHandler):
    ...
    def process(self):
        ...
        # EP閲覧フラグを立てる.
        if BackendApi.check_battleevent_lead_epilogue(model_mgr, uid, cur_eventmaster.id, using=settings.DB_READONLY):
            flagrecord = BackendApi.update_battleevent_flagrecord(cur_eventmaster.id, uid, epvtime=now)
            model_mgr.set_got_models([flagrecord])
        
        url = UrlMaker.battleevent_top(cur_eventmaster.id)
