...
class HappeningHandler(AppHandler):
    ...
    def getHappeningRaidSet(self):
        ...
        if self.__happeningraidset is None:
            ...
            happeningid = BackendApi.get_current_happeningid(model_mgr, v_player.id, using=settings.DB_READONLY)
            if happeningid:
                happeningraidset = BackendApi.get_happeningraidset(model_mgr, happeningid, using=settings.DB_READONLY)
    ...
    def writeHtmlSwitchEvent(self, name, eventid=None, eventmaster=None, basedir_normal='happening'):
