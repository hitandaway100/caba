...
class Handler(RaidEventBaseHandler):
    ...
    def process(self):
        ...
        args = self.getUrlArgs('/raideventscoutresultanim/')
        try:
        ...
        if not proc:
            url = UrlMaker.raidevent_scoutresult(stageid, scoutkey)
            self.appRedirect(self.makeAppLinkUrlRedirect(url))
            return
        
        if next_event and table.has_key(next_event.get_type()):
            url = UrlMaker.raidevent_scoutresultanim(stageid, scoutkey, index+1)
        else:
            url = UrlMaker.raidevent_scoutresult(stageid, scoutkey)
