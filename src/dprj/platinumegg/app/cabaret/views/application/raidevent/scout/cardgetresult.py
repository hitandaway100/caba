...
class Handler(RaidEventBaseHandler):
    ...
    def process(self):
        args = self.getUrlArgs('/raideventscoutcardgetresult/')
        try:
        ...
        if target_event is None:
            ...
            url = self.makeAppLinkUrlRedirect(UrlMaker.raidevent_scouttop())
            self.appRedirect(url)
            return
        elif not target_event.is_received:
            if settings_sub.IS_LOCAL:
                raise CabaretError(u'まだ女の子をスカウトしていません')
            url = self.makeAppLinkUrlRedirect(UrlMaker.raidevent_scoutresult(stageid, playdata.alreadykey))
            self.appRedirect(url)
            return
        ...
        # ステージ.
        arr = BackendApi.get_raidevent_stagemaster_list(model_mgr, [stageid], using=using)
