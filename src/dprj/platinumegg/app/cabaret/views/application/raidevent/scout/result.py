...
class Handler(RaidEventBaseHandler):
    ...
    def process(self):
        args = self.getUrlArgs('/raideventscoutresult/')
        try:
        ...
        if playdata and playdata.confirmkey == scoutkey:
            # DBからとり直すべき.
            playdata = BackendApi.get_raideventstage_playdata(model_mgr, mid, uid, using=using, reflesh=True)
    ...
    def procLevelup(self, mid, stagemaster, playdata, event):
        ...
        if event.level != v_player.level:
            url = UrlMaker.raidevent_scoutdo(stagemaster.id, playdata.confirmkey)
    ...
    def __putCompleteData(self, model_mgr, playdata, stagemaster):
        ...
        if 0 < stagemaster.boss and BackendApi.check_event_boss_playable(playdata, stagemaster):
            ...
            nextstagemaster = BackendApi.get_raidevent_next_stagemaster(model_mgr, playdata.mid, stagemaster, using=settings.DB_READONLY)
            if nextstagemaster and nextstagemaster.stage != stagemaster.stage:
                ...
                url = UrlMaker.raidevent_scoutdo(nextstagemaster.id, playdata.confirmkey)
            else:
                url = UrlMaker.raidevent_scoutdo(stagemaster.id, playdata.confirmkey)
    ...
    def procGetCard(self, mid, stagemaster, playdata, event):
        ...
        if event.is_received:
            ...
            self.appRedirect(self.makeAppLinkUrlRedirect(UrlMaker.raidevent_scoutcardgetresult(stagemaster.id)))
            return
        ...
        # 獲得判定へ飛ぶ.
        url = UrlMaker.raidevent_scoutcardget(stagemaster.id)
        self.html_param['url_exec'] = self.makeAppLinkUrl(url)
        
        # アイテム.
        BackendApi.put_scoutcard_uselead_info(self, UrlMaker.raidevent_scoutcardget(stagemaster.id))
    ...
    def procApNone(self, mid, stagemaster, playdata, event):
        ...
        self.setFromPage(Defines.FromPages.RAIDEVENTSCOUT, (stagemaster.id, urllib.quote(playdata.confirmkey, '')))
