# -*- coding: utf-8 -*-
import settings
from platinumegg.app.cabaret.kpi.models.scoutevent import ScoutEventTanzakuHash
...
class Manager(ScoutEventCSVBase):
    ...
    def get_data(self):
        ...
        data = ScoutEventTanzakuHash.aggregate(uid_max)
        self.__data = data
        return data or None
    ...
    def delete(self):
        ...
        redisdb = ScoutEventTanzakuHash.getDB()
        keylist = []
        for arr in self.__data:
            uid = arr[0]
            keylist.append(ScoutEventTanzakuHash.makeKey(uid))
            if 100 <= len(keylist):
