...
class Command(BaseCommand):
    ...
    ●ディレクトリになっていたらその中全部実行.
        -> 複数パターン確認するため.
    """
    MODULE_BASE = 'platinumegg.test'
    IGNORES = ['__init__','base','dummy_factory',
               'pc','item_use.boxexpansion','item_use2.boxexpansion',
               'battleeventtestbase','battleeventexplain.prizes_point',
               'raideventtimebonus', 'happeningdo',
               'raideventtest', 'util',
               'raideventexplain.faq','sceventexplain.faq',
               
               'promotioncheck','promotionconditionget',    # ここはクロスプロモがあるときだけでいい.
               'sceventexplain.performance',        # 短冊がないときは動かない.短冊を復活させるなら消す.
               ]
    ...
    def handle(self, *args, **options):
        ...
        msg += 'ex)target loc="test/battleEnter/win", test name="battleEnter.win"\n'
        msg += 'test name:'
        resp = raw_input(msg)
...
#                print '%s:%s' % (k,v)
            ...
            for filename in files:
                if filename[-3:] == '.py':
                    ...
                    # パッケージらしきもの.
                    if filename not in Command.IGNORES:
    ...
    def __run_package(self, package_name, api):
        ...
        for filename2 in filenames2:
            if filename2[-3:] == '.py':
                ...
                package_module_name = '%s.%s' % (package_name, module_name)
                if module_name not in IGNORES and not package_module_name in Command.IGNORES:
