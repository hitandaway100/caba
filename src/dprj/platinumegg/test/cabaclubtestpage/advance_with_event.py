...
class ApiTest(CabaclubTest):
    ...
    def setUp(self):
        ...
        storeplayerdata.etime = storeplayerdata.utime - datetime.timedelta(seconds=Defines.CABARETCLUB_STORE_EVENT_INTERVAL - self.__storemaster.customer_interval)
    ...
    def check(self):
        ...
        elif storeplayerdata.etime.strftime("%Y%m%d%H%M%S") != (self.__cabaclub_dummy.now - datetime.timedelta(seconds=(self.__storemaster.customer_interval * (self.__exec_cnt - 1)) % Defines.CABARETCLUB_STORE_EVENT_INTERVAL)).strftime("%Y%m%d%H%M%S"):
        ...
        customer, proceeds = self.calcCustomerAndProceeds(self.__cabaclub_dummy.master, self.__storemaster, self.__exec_cnt, self.__storeplayerdata.scoutman_add, self.__cabaclub_dummy.cardlist, self.__eventmaster.customer_up*self.__exec_cnt, self.__eventmaster.proceeds_up*self.__exec_cnt)
