...
class ApiTest(ApiTestBase):
    ...
    def setUp(self):
        ...
        # シャンパン情報.
        champagnedata = self.create_dummy(DummyType.RAID_EVENT_CHAMPAGNE, self.__player1.id, eventmaster.id, 0, etime=now + datetime.timedelta(seconds=eventmaster.champagne_time))
        ...
        eventflagrecord = self.create_dummy(DummyType.RAID_EVENT_FLAGS, self.__eventmaster.id, self.__player1.id, tbvtime=config.starttime)
    ...
    def get_urlargs(self):
        """URLでAPIに送る引数.
        """
        return u'/do/%d/%s' % (self.__raid.id, urllib.quote(self.__player1.req_confirmkey, ''))
    ...
    def check(self):
        ...
        # イベントポイント.
        scorerecords = BackendApi.get_raidevent_scorerecords(model_mgr, self.__eventmaster.id, [self.__player0.id, self.__player1.id])
        ...
        elif len(arr) != 1 or sum(arr) != scorerecord0.point or sum(arr) != scorerecord0.point_total:
        ...
        elif sum(arr) != scorerecord1.point or sum(arr) != scorerecord1.point_total:
