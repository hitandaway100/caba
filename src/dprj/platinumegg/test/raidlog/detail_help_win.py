...
class ApiTest(ApiTestBase):
    ...
    def setUp(self):
        ...
        deck = BackendApi.get_deck(self.__player1.id)
        deckcardlist = BackendApi.get_cards(deck.to_array())
        friendcard = BackendApi.get_leaders([self.__player0.id]).get(self.__player0.id)
        
        model_mgr = ModelRequestMgr()
        BackendApi.tr_raidbattle(model_mgr, raidboss.id, self.__player1.req_confirmkey, self.__player1, raidmaster, deckcardlist, friendcard, False)
        model_mgr.write_all()
        model_mgr.write_end()
        
        self.__raidlog = BackendApi.get_raidlog_by_raidid(model_mgr, self.__player1.id, raidboss.id)
