...
class ApiTest(ApiTestBase):
    ...
    def setUp(self):
        ...
        self.__itemmaster = self.create_dummy(DummyType.ITEM_MASTER, mid=Defines.ItemEffect.CABACLUB_BARRIER, evalue=30)
        self.__item = self.create_dummy(DummyType.ITEM, self.__player, self.__itemmaster, vnum=self.__usenum, rnum=1)
        # キャバクラのアイテム使用状況.
        self.__itemdata = self.create_dummy(DummyType.CABA_CLUB_ITEM_PLAYER_DATA, self.__player.id, barrier_time=self.__now)
    ...
    def check(self):
        ...
        if itemdata.barrier_id != self.__itemmaster.id:
            raise AppTestError(u'CabaClubItemPlayerDataにアイテムIDが正しく設定されていない')
        elif itemdata.barrier_time < (self.__itemdata.barrier_time + datetime.timedelta(seconds=3600*6)):
