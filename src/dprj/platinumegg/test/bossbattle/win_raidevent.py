...
class ApiTest(RaidEventApiTest):
    ...
    def check(self):
        playdata = RaidEventScoutPlayData.getByKey(RaidEventScoutPlayData.makeID(self.__player0.id, self.__stagemaster.eventid))
        if playdata is None:
            raise AppTestError(u'クリアフラグが保存されていない')
        
        result = BackendApi.get_bossresult(ModelRequestMgr(), self.__player0.id, self.__stagemaster.id)
        if result is None:
            raise AppTestError(u'結果が保存されていない')
        
        if not BackendApi.get_present_idlist(self.__player0.id):
