...
class ApiTest(SugorokuApiTest):
    ...
    def setUp(self):
        ...
        # 停まるマス.
        self.update_squares_master(mapdata.mass_dict[7], event_type=Defines.SugorokuMapEventType.JUMP, event_value=2, prize=[prize.id])
        self.update_squares_master(mapdata.mass_dict[2], event_type=Defines.SugorokuMapEventType.GO, event_value=1, prize=[prize.id])
        # ログインボーナス.
    ...
    def check(self):
        ...
        self.check_playerdata(playerdata, 3, [self.__mass_dict[loc].id for loc in (1,7,2,3)], self.__present_num + 2)
