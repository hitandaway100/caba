    ...
    def createRaidLog(self, player0, happeningstate, is_help):
        ...
        if happeningstate == Defines.HappeningState.CANCEL:
            ...
            if happeningstate == Defines.HappeningState.END:
                ...
                if is_help:
                    model_mgr = ModelRequestMgr()
                    happeningset = BackendApi.get_happening(model_mgr, happening.id)
                    raidboss = BackendApi.get_raid(model_mgr, raidboss.id)
                    BackendApi.tr_happening_end(model_mgr, happeningset.happening, raidboss)
                    model_mgr.write_all()
                    model_mgr.write_end()
